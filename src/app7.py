...
if uploaded_file is None and selected_tab == "⚖️ Compare Targets":
    section_title("⚖️", "COMPARE UP TO 5 GALAXY TARGETS")
    st.markdown("Upload **2–5 galaxy images** and compare their morphology, AI confidence, anomaly score, alert level and basic image information.")
    comparison_files = st.file_uploader("Add galaxy images to compare", type=["jpg", "jpeg", "png"], accept_multiple_files=True, key="comparison_uploader_empty", help="Maximum 5 images.")
    if len(comparison_files) > 5:
        st.warning("⚠️ Maximum is 5 images. Only the first 5 will be analysed.")
    ...
    if comparison_files:
        ...
        cols=st.columns(min(3,len(results)),gap="large")
        ...
        st.info("🔭 Add up to 5 galaxy images above to activate the comparison scanner.")
    st.stop()
...
if selected_tab == "🛰️ Telemetry":
    ...
    st.markdown("Upload **2–5 galaxy images** to compare them using the same ResNet50 morphology classifier and convolutional autoencoder.")
    comparison_files = st.file_uploader("Add galaxy images to compare", type=["jpg","jpeg","png"], accept_multiple_files=True, key="comparison_uploader_main", help="Maximum 5 images.")
    if len(comparison_files)>5:
        st.warning("⚠️ Maximum is 5 images. Only the first 5 will be analysed.")
        comparison_files=comparison_files[:5]
    if comparison_files:
